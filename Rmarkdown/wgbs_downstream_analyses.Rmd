...
All these methods are based on analysing $\mathbf{m}$ and $\mathbf{u}$
...
The idea of testing for differential methylation at _a priori_ defined regions is relatively straightforward. Suppose we have a two-group experiment and let $\widebar{B_{r}^{k}}$ be the true average level of methylation for the $r^{th}$ region in the $k^{th}$ group ($k = 0, 1$). The null hypothesis is $H_{0}: \widebar{B_{r}^{0}} = \widebar{B_{r}^{1}}$ against the alternative hypothesis $H_{1}: \widebar{B_{r}^{0}} \neq \widebar{B_{r}^{1}}$.
