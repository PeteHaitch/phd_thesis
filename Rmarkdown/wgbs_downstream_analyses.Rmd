...
- `QDMR` \citep{Zhang:2011dp} and `CpG_MPs` \citep{Su:2012hl}, developed by the same research group, are methods for identifying DMRs using a form of methylation entropy from MeDIP-chip and BS-seq, respectively.
- \citet{Xie:2011cy} introduced the concept of methylation entropy for BS-seq.


While mathematically based on the idea of Shannon entropy \citep{Shannon:IH1_srIe}, the interpretation of methylation entropy has much in common with the physical concept of "disordered/heterogeneity"
...
A common thread in Section \ref{sec:downstream_analyses_of_m-tuples} is that while interesting methods have been proposed to address these questions, the software implementations are somewhat lacking. This in part motivated me to begin developing `MethylationTuples`, an R package for managing, analysing and visualising methylation patterns at m-tuples.

- I'm using to address co-methylation
- Could be used for entropy, epipolymorphism and epialleles.
- Could implement Fang or Peng's method for ASM
