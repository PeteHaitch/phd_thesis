...
\begin{table}[h]
\centering
\caption[Proposed multiple hypothesis testing procedures]{Methods proposed for adjusting for the multiple hypothesis testing performed in an analysis of DMCs. Several papers use describe their analysing as performing a ``false discovery rate adjustment'' or ``false discovery rate correction'' without explicitly stating what they are doing or citing a reference. One paper uses an ultra-conservative Bonferonni correction.}
...
\begin{table}[h]
\centering
\caption[Size of \texttt{MethPat} objects for the \emph{EPISCOPE} data]{Size of \texttt{MethPat} objects for the \emph{EPISCOPE} data. All m-tuples are stranded. The `size' of the \texttt{MethPat} object, reported in gigabytes (GB), is computed using the \texttt{pryr::object\_size()} function (\url{http://cran.r-project.org/web/packages/pryr/index.html}). The `number of rows' corresponds to the number of m-tuples in the object. The `number of assays' is $2^{\text{m}}$, where m is the size of the m-tuples. The final column is a measure of how sparse the data are --- a $0$ values means that particular methylation pattern was not observed in that particular sample and an \texttt{NA} value means that that m-tuple was not observed in that particular sample.}
