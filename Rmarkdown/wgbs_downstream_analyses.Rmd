...
- methclone
- Conrad (MPFE)
- How it differs to Emma Whitelaw's definition

\section{`MethylationTuples`}\label{sec:MethylationTuples}

A common thread in Section \ref{sec:downstream_analyses_of_m-tuples} is that while interesting methods have been proposed to address these questions, the software implementations are rather lacking. `MethylationTuples` is an R package for managing, analysing and visualising methylation patterns at m-tuples.  While initially developed to support my research into co-methylation (Chapter \ref{chap:co-methylation}), `MethylationTuples`, in conjunction with `methtuple`, can also be used to do many of the above analyses.

- I'm using it to address co-methylation
- Could be used for entropy, epipolymorphism and epialleles.
- Could implement Fang or Peng's method for ASM, but less readily the "heterozygous SNP" method
