...
\subsection{Partially methylated domains}\label{sec:pmds}

__UP TO HERE: Re-write as "Epigenome segmentation"__

Partially methylated domains, PMDs, are long stretches of the genome where the average methylation level, $B_{i}$, is away from the extremes of $0$ and $1$, typically in the range $0.20 - 0.70$. PMDs were first identified in the IMR90 genome by \citet{Lister:2009hy}. Using a simple sliding window algorithm, \citeauthor{Lister:2009hy} unexpectedly found that approximately $40\%$ of every autosome was a PMD and that the average length of these PMDs was a large 153 kb. They also showed that these PMDs were not simply due to a methylated population and an unmethylated subpopulation of cells in the sample. This did this by showing that inidividual reads mapped to these PMDs contained both methylated and unmethylated bases.
...
[^other_definitions]: This is closely related to the idea of identifying epialleles, for which methylation entropy has also played a role \citep{Li:2014ei} and which I discuss in Section \ref{sec:epialleles_and_epipolymorphism}. Methylation entropy has also been used to identify differential methylation, however, I do not discuss this further since this uses a different definition that is not based on analysing methylation patterns at m-tuples \citep{Zhang:2011dp, Su:2012hl}.
...
Regardless of where you draw the line as to what constituted an epiallele, it has become clear that the occurrence of multiple methylation patterns at an m-tuple is the norm rather than the exception when performing bisulfite-sequencing \citep{Landan:2012kp}. There has been some progress in developing software to extract and analyse these epialleles, which I now describe.
...
A common thread in Section \ref{sec:downstream_analyses_of_m-tuples} is that while interesting methods have been proposed to address these questions, the software implementations are often lacking or, at best, very much tailored toward a particular analysis. What has been lacking is:
