...
__UP TO HERE: Read the `methclone` paper and say somethign intelligent about it. Then, finish the Methods section and move onto the `MethylationTuples` section.__
...
- `MPFE`
- `methpat`
- ASM-methods are arguably epiallele detection methods
- Methods based solely on methylation patterns don't check that the sequence is identical.

\section{`MethylationTuples`}\label{sec:MethylationTuples}

A common thread in Section \ref{sec:downstream_analyses_of_m-tuples} is that while interesting methods have been proposed to address these questions, the software implementations are rather lacking or, at the least, very much tailored toward a particular analysis. What has been lacking is a general method for extracting methylation patterns at m-tuples and software for manipulating, analysing and visualising the results. I have made significant progress towards the first goal with `methtuple` (see Section \ref{sec:methtuple}) and now introduce `MethylationTuples` ([https://github.com/PeteHaitch/MethylationTuples](https://github.com/PeteHaitch/MethylationTuples)) to address the second missing link.

`MethylationTuples` is an R package for managing, analysing and visualising methylation patterns at m-tuples.  While initially developed to support my research into co-methylation (Chapter \ref{chap:co-methylation}), the data structures developed in `MethylationTuples` are also well suited to analysing methylation entropy, allele-specific methylation, epialleles and epipolymorphism
...
__TODO: Things to mention__
- `GenomicTuples`
  - Package build on existing class-leading infrastructure
- Describe utility functions, e.g., `collapseStrand`
- Analytical methods currently implemented in the package
- Compatability with other packages, e.g., `bsseq`.
- Challenges
- Future directions
