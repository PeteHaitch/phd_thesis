...
Firstly, the ``possible range of the [log] odds ratio is unaffected by the marginal distributions'' (__CITE: quote from \cite{Kraemer:2004el} but references two other papers. Find original quote__). Another way of putting this is that given an odds ratio and a set of marginal probabilities, there exists one and only one $2 \times 2$ table satisfying these constraints (__CITE: Plackett?__). In contrast, Pearson's correlation of a $2 \times 2$ is restricted by the marginal probabilities (__CITE: Kraemer HC. A measure of 2 × 2 association with stable variance and approximately normal small sample distribution: planning cost-effective studies.__), which I find undesirable.
...
As discussed in Section \ref{sec:m-bias}, the incomplete removal of M-bias is a real problem for reads that have undergone pre-trimming because this destroys the relationship between read-position and sequencing cycle. __FIGURE__ shows that there is considerable variation in the read lengths of the _ADS_ sample, indicative of aggressive pre-trimming of these reads (__TODO: Histogram of read lengths__).
