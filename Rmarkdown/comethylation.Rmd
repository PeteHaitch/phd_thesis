...
\label{fig:Ziller_merged_spearman_beta_cors_min_cov_10_strand_collapsed_CI}
\end{figure}
...
Figures \ref{fig:EPISCOPE_spearman_beta_cors_min_cov_10_strand_collapsed_CI}, \ref{fig:Lister_spearman_beta_cors_min_cov_10_strand_collapsed_CI}, \ref{fig:Seisenberger_spearman_beta_cors_min_cov_10_strand_collapsed_CI} and \ref{fig:Ziller_merged_spearman_beta_cors_min_cov_10_strand_collapsed_CI} are plots of Spearman correlations of $\beta$-values for pairs of CpGs stratified by whether the pair is inside or outside of a CpG island[^spanning]. Again, the substantial noise in these plots is due to a lack of $NIL = 0$ pairs with large $IPD$s, particularly inside CpG islands.
...
- Complications of interpreting NIL >= 0 plots
- Periodicities or lack thereof
- CGI vs. non-CGI
...
\label{fig:Ziller_merged_spearman_beta_cors_all_min_cov_10_strand_collapsed_CI}
\end{figure}


\subsection{Summary}

The $NIL = 0$ results measure the 'first-order' or 'one-step' correlations of $\beta$-values whereas the $NIL \geq 0$ measure a mixture of different orders of correlations of $\beta$-values. For this reason I find the $NIL = 0$ plots are easier to interpret, however, which is more relevant will depend on the application.
