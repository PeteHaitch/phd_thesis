...
The $150-200$ bp periodicity is also evident from the $NIL \geq 0$ data and is particularly clear for the CpG pairs outside of CpG islands. __TODO: Discuss more__

\subsection{Summary}

The $NIL = 0$ results measure the 'first-order' or 'one-step' correlations of $\beta$-values whereas the $NIL \geq 0$ measure a mixture of different orders of correlations of $\beta$-values. For this reason I find the $NIL = 0$ plots are easier to interpret, however, they do not tell the whole story.

By comparing the $NIL = 0$ and $NIL \geq 0$ results we see that the strength of the correlation depends on not just the distance between the two CpGs but the number of intervening CpGs. This indicates that these correlations are more than 'first-order'. Moreover, the differences in correlations between CpG islands and non-islands indicates that the 'order' of these correlations itself varies across the genome. All this means that the vector of $\beta$-values for a single sample forms a complicated and highly non-stationary process with variable-order correlations.

This is valuable information in trying to account for correlated $\beta$-values in downstream analyses. However, it is based on multiple levels of aggregation, such as aggregation of cells to form $\beta$-values and such aggregations may hide the source of these correlated $\beta$-values. To better understand what leads to correlated $\beta$-values, I investigated within-fragment co-methylation.
...
There are several alternative measures of association for a $2 \times 2$ table of binary variables such as __EXAMPLES__
...
- Look for tables with LOR > 0 and r = 0 (and vice versa?)
...
- Discuss 1D nature of $IPD$s vs. $4D$ structure of genome (3D + time)
