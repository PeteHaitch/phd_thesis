...
__TODO: Write up proof in appendix and link to it__

__TODO: What is the general restriction on $c$ when $p_{h, i} \neq p_{h', i'}$?__

In general, $PIM^{*} \in [0, 1]$. However, there are a few important special cases:

__TODO: Derive results for special cases and illustrate them with a plot__

1. $z_{h, i}$ Bernoulli($p_{h}$) and $z_{h, i'}$ Bernoulli($p_{h'}$) and with $cor(z_{h, i}, z_{h', i'}) = c = 0$.
2. $z_{h, i}$ and $z_{h', i'}$ both Bernoulli($p$), $p = p_{h, i} = p_{h', i'}$, and $cor(z_{h, i}, z_{h', i'}) \neq c$.
3. $z_{h, i}$ and $z_{h', i'}$ both Bernoulli($p$), $p = p_{h, i} = p_{h', i'} > 0.5$, and $cor(z_{h, i}, z_{h', i'}) \neq c$.
4. $z_{h, i}$ and $z_{h', i'}$ both Bernoulli($p$), $p = p_{h, i} = p_{h', i'}$, and $cor(z_{h, i}, z_{h', i'}) = c$.

__TODO START: Re-write and simplify these special case results__
For case 4 we have:

\begin{align}
\label{eq:pim_iid}
PIM_{1}^{*} &= Pr(z_{h, i} = z_{h', i'}) \\
          &= Pr(Pr(z_{h, i} = 0, z_{h', i'} = 0)) + Pr(Pr(z_{h, i} = 1, z_{h', i'} = 1)) \\
          &= (1 - p)^2 + p^2 \\
\end{align}

By differentiating $PIM_{ind}^{*}$ with respect to $p$ we see that $PIM_{iid} \geq 0.5$.

For case 2 we have:

\begin{align}
\label{eq:pim_inid}
PIM_{2}^{*} &= Pr(z_{h, i} = z_{h', i'}) \\
          &= Pr(Pr(z_{h, i} = 0, z_{h', i'} = 0)) + Pr(Pr(z_{h, i} = 1, z_{h', i'} = 1)) \\
          &= (1 - p_{h}) (1 - p_{h'}) + p_{h} p_{h'} \\
\end{align}

where $p_{h}$ (resp. $p_{h'}$) is the genome-wide average CpG methylation level for the sample from which the $h^{th}$ (resp. $h'^{th}) arose. Unlike $PIM_{iid}$, $PIM_{inid}$ may be less than 0.5. In fact, $0 \leq PIM_{inid} \leq 1$ for $0 \leq p_{h}, p_{h'} \leq 1$.
__TODO END: Re-write and simplify these special case results__

The plot of "percentage identical methylation" in Figure 3a of \cite{Eckhardt:2006gh} asymptotes at 0.4, which is less than the theoretical lowerbound under the assumption of indepdence \eqref{eq:pim_ind}. This is likely due to the fact that the amplicons, and thus the $z_{h, i}$, come from a variety of samples and genomic contexts with different average CpG methylation levels, $p$. Alternatively, it may be that it is not $PIM^{*}$ that is plotted in Figure 3a or that the methylation states of two CpGs separated by a large distance are anti-correlated, resulting in $PIM^{*} < 0.5$ (__TODO: Ref equation__).

__TODO__

* Describe some analyses using autocorrelation of beta-values.
* More recently, within-fragment analyses. Describe concept (move from next section to this section).
* Other definitions/methods
