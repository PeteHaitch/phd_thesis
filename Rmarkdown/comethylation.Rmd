...
- Use parameters $n, \pi, \psi, \phi$ and not the ones from Chapter \ref{chap:wgbs_statistical_framework} since these are more succinct.
...
Furthermore, the marginal probabilities of these tables, $\pi_{\cdot 1} = 1 - \pi_{\cdot 2}$ and $\pi_{1 \cdot} = 1 - \pi_{2 \cdot}$, will be very often close to the boundaries of zero and one. This is because in this case the marginal probability represents the probability that the methylation locus is methylated, and we have seen in Chapter \ref{chap:wgbs_statistical_framework} that these values, as estimated by $\beta$-values, have a bimodal distribution with each mode near the boundary.
...
# truncation specified by 'trunc'. It uses a more explicit approach to simulate # truncated RVs than rtpois() because the same trick doesn't immediately work
# for NB-distributed RVs.
rtnbinom <- function(n, mu, size, trunc) {
