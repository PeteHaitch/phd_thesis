...
4. $IPD$-CGI-chromosome: A $2 \times 2 \times K$ table for each $IPD$, chromosome and CGI-status combination.
...
We will first compare the results of chromosome-level estimates to genome-level estimates.

\subsubsection{Chromosome-level analyses}

__UP TO HERE: Plots should have $\widehat{\theta_{MH}}$, not $\psi$, on the y-axis__

__FIGURES__ show the results for the _EPISCOPE_, _Lister_, _Seisenberger_ and _Ziller_ datasets, respectively. The estimated log odds ratio, $\widehat{\theta_{MH}}$ is plotted as a function of $IPD$. The genome-level estiamtes are shown by the blue line while the chromosome-level estimates are plotted as points coloured by whether the chromosome is an autosome, X chromosome, Y chromosome or mtDNA.

#### These are the main points to write about

- chromosome-level and genome-level qualitatively similar.
- Chromosome-variation indicates uncaptured heterogeneity. Difficult to get at.
- point out interesting chromosomes


\subsubsection{Genome-wide analyses}
...
__OLD STUFF BELOW HERE__

I compute these genome-wide and after stratifying CpG 2-tuples by whether they are in a CpG island. These analyses are performed separately for each strand and also for strand-collapsed counts of methylation patterns.
