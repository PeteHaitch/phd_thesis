...
Chapter \ref{chap:co-methylation_review} documented several previous analyses based on 'correlations' of aggregate methylation levels, such as $\beta$-values. What was lacking from these was a clear description and implementation of the proposed method. Here I describe two simple methods based on correlations of $\beta$-values. Both of these are implemented in the `methLevelCor` function that is part of the `MethylationTuples` software.
...
\subsection{Constructing the pairs}
...
\subsection{Stratifying by a genomic feature}
...
\subsection{Using pairs of methylation loci}\label{subsec:pairs}
