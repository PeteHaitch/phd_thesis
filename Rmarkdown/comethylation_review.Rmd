...
To date, most analyses of co-methylation have been based on analysing correlations of aggregate levels of methylation, such as $\beta$-values, within individual samples \citep{Eckhardt:2006gh, Cokus:2008fc, Li:2010fb, Lyko:2010dr, Lacey:2013iy}. An exception is the analysis in \citet{Lister:2009hy}. While it also uses within-sample aggregate methylation levels, rather than estimating correlations of the methylation levels, the authors instead look for patterns in the spacing of 'methylcytosines' along the genome[^Lister_methylcytosines].

[^Lister_methylcytosines]: See Section \ref{sec:beta-value_interpretation} for a critique of this definition of 'methylcytosine'.
...
While I don't have the data used in \citet{Lyko:2010dr}, I believe an example using the chromosome 22 data from the _FF_ sample demostrates the point. Let $\mathbf{\beta^{\ast}}$ be the augmented vector of $\beta$-values, where:
...
__FIGURE__ is a plot of the autocorrelation of the augmented vector, $\mathbf{\beta^{\ast}}$.

__TODO: Include plot of this for FF data on chr22. This brings into question the other results based on this plot, e.g., the 3bp periodicity, which may reflect nucleotide periodicities rather than methylation periodicities.__
__TODO: What does this plot look like__
__TODO: What are the implications for the results in Lyko__
...
\citet{Lister:2009hy} report an $8-10$ nucleotide periodicity in the occurrence "methylcytosines". Recall that \citet{Lister:2009hy} define a "methylcytosine" as a cytosine that has a non-zero $\beta$-value, as determined by a Binomial test (see Section \ref{sec:beta-value_interpretation} for details and a criticism of this concept). Hence, while this may appear to be a within-fragment analysis of co-methylation with it's reference to "methylcytosines", it in fact  based on $\beta$-values. Furthermore, unlike the methods described in Section \ref{sec:previous_cor_of_agg_methylation}, the analysis is not based on a correlation of these $\beta$-values, but rather on unnormalised counts of a binary classification of $\beta$-values, which leads to problems.
...
- Re: Autocorrelation analyses of the $\mathbf{\beta} = (\beta_{1}, \ldots, \beta_{N_{loci}})$. The interpretation of these analyses is complicated by the non-stationarity of $\mathbf{\beta}$, meaning that the strength of the dependence is affected not only by the distance between loci but also by their positions in the genome.
- Comment on http://nar.oxfordjournals.org/content/40/22/11490.long
- Not accounting for autocorrelation of errors, which manifest as autocorrelation of the residuals: "While it does not bias the OLS coefficient estimates, the standard errors tend to be underestimated (and the t-scores overestimated) when the autocorrelations of the errors at low lags are positive." [http://en.wikipedia.org/wiki/Autocorrelation](http://en.wikipedia.org/wiki/Autocorrelation)
