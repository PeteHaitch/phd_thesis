...
\subsubsection{Multi-group experiments}\label{sec:two_group_dmcs}
...
1. Usinb regions that are defined _a priori_, which are then tested for differential methylation. Such regions might be CpG islands (__CITE__), MspI restriction fragments (__CITE__) gene promoters (__CITE__) or predefined bins (e.g. 3bp tiles [Bing Ren] or 100bp tiles \citep{Park:2014ho}).
2. Using data-driven regions, e.g., defined from an analysis of DMCs, which are then tested for differential methylation.

The former is simple to analyse but is limited in its ability to discover new differentially methylated regions. The latter offers the opportunity to identify previously unknown regions but valid statistical inference of these regions is __very challenging__.
