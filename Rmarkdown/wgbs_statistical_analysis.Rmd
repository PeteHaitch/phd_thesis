...
A common downstream analysis is the identification of differential methylation. This includes the identification of differentially methylated cytosines (DMCs) and differentially methylated regions (DMRs). Many methods have been proposed \citep[e.g.,]{Akalin:2012cmChen:2013eh, Chen:2014jb, Dolzhenko:2014bo, Gokhman:2014cp, Jaffe:2012gx, Lacey:2013iy, Lister:2009hy, Rijlaarsdam:2014hp, Sofer:2013bk, Xie:2014ez, Feng:2014iq, Hebestreit:2013ko, Sun:2014fk, Park:2014ho, Hansen:2012gr}. All of these methods are designed for two-group experiments, with some being general enough to extend to multiple-group experiments[^multi-group_extension]. I describe in greater detail methods for identifying differential methylation in a one-group experiment, admitedly an unusual design but one that I faced during my PhD.
...
There are many other questions that can be addressed using bisulfite-sequencing data. Like differential methylation, some of these might be also be tackled with other methylation assays, such as microarrays. However, as we will see, some can only be answered by assays that have single nucleotide resolution, which is where bisulfite-sequencing can prove particularly powerful.
...
PMDs were first identified in the IMR90 genome by \citet{Lister:2009hy}. Using a simple sliding window algorithm, \citeauthors{Lister:2009hy} unexpectedly found that approximately $40\%$ of every autosome was a PMD and that the average length of these PMDs was a large 153 kb. They also showed that these PMDs were not simply due to a methylated population and an unmethylated subpopulation of cells in the sample. This did this by showing that inidividual reads mapped to these PMDs contained both methylated and unmethylated bases.
...
Such an approach, while straightforward, is also obviously limited to a subset of the available methylation loci. Both \citet{Fang:2012ci} and \cite{Peng:2012dh} published methods to detect allele-specfic methylation that do not require nearby heterozygous genetic variants nearby to the methylation locus of interest. These methods rely on the probabilistic assignment of reads to alleles, which are treated as missing data. Unfortunately, there is no publicly available software implementing the method proposed by \cite{Peng:2012dh} and so I do not discuss it further.
