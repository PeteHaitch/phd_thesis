...
\caption{Kernel density estimates of the genome-wide distribution of CpG $\beta$-values for the EPISCOPE data. Observations have been combined across strands and only CpGs with at least $10\times$ sequencing coverage are included. Most CpGs are highly unmethylated or highly methylated, with the buffy coat samples being the most extreme example of this bimodality. 'Spikes' in the density estimate are due to the discreteness of $\beta$-values. Abbreviations: \em{BUF} = buffy coat; \em{SA} = subcutaneous adipose; \em{VA} = visceral adipose; \em{VAT} = adipocytes derived from visceral adipose.}
\label{fig:EPISCOPE_beta_distribution}
\end{figure}

\begin{figure}[h]
\includegraphics[width=\textwidth]{../figures/EPISCOPE_beta_distribution_CGI_stratified.pdf}
\caption{Kernel density estimates of the genome-wide distribution of CpG $\beta$-values for the EPISCOPE data, stratified by whether the CpG is in a CpG island. Only CpGs with at least $10\times$ sequencing coverage are included. Most CpGs outside of CpG islands are methylated whereas most CpGs within CpG islands are unmethylated. Spikes' in the density estimate are due to the discreteness of $\beta$-values. Abbreviations: \em{BUF} = buffy coat; \em{SA} = subcutaneous adipose; \em{VA} = visceral adipose; \em{VAT} = adipocytes derived from visceral adipose; \em{CGI} = CpG island.}
...
The idea of testing for differential methylation is relatively straightforward. For each sample, the region-wise methylation level is some summary of the methylation level at each loci in that region, for example, the weighted average of $\beta$-values for all loci in the region, where the weights are proportional to the sequencing coverage. These averages are then compared between samples. I describe basic implementations of this for both one-group and multi-group experiments, and then detail a more sophisticated analysis implemented in `BiSeq` \citep{Hebestreit:2013ko}.
...
\subsubsection{Multi-group experiments}\label{sec:multi_group_dmrs}

__TODO__

\subsubsection{A more complex example}\label{sec:a_more_complex_example}
...
\section{Other downstream analyses}
