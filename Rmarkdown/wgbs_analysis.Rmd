...
```{r m-bias, eval = TRUE, echo = FALSE, message = FALSE, fig.lp = "fig:ADS_M-bias", fig.cap = "M-bias plot for the ADS sample from the Lister (2011) dataset. Each of read 1 (R1) and read 2 (R2) are plotted separately. We see significant CpG M-bias at the start of read 2 and some noise at the start of read 1.", fig.width = 8}
library(dplyr)
library(ggplot2)
...
However, occasionally there are spikes in the M-bias plot, which indicate specific read-positions that we would like to exclude. Figure \ref{fig:E18VA_M-bias} shows an example of such a spike that occurred at read-position 101. Using `bismark_methylation_extractor` we would be forced to either retain this position or ignore read-positions $101-150$, effectively ignoring one third of the sequencing data, much of it unaffected by M-bias. My `methtuple` software is more flexible by allowing the user to exclude individual read-positions or ranges of read-positions via the `--ignore-read1-positions` and `--ignore-read2-positions` flags.
...
[^bismark_methylation_extractor]: `Bismark` also includes a program called `bismark_methylation_extractor`, which, as the name suggests, extracts the methylation calls from the `SAM/BAM` file. So while `Bismark` performs methylation calling as part of the alignment, a secondary step using `bismark_methylation_extractor` is required to make use of the methylation calls. `methtuple` may be used as a drop-in replacement for `bismark_methylation_extractor` and provides several extra features, discussed later in this section.

Most methylation callers, such as `Bismark` and `Bis-SNP`, perform methylation calling at single methylation loci, which I refer to as 1-tuples. In contrast, `methtuple` can perform methylation calling at m-tuples; an m-tuple is $m$ adjacent methylation loci, where $m = 1, 2, \ldots$. m-tuples are discussed in greater detail in Chapter \ref{chap:a_statistical_model_of_wgbs} and are used extensively in Chapter \ref{chap:comethylation}. One of the few other methylation callers that can perform calling at m-tuples is `methpat` ([https://github.com/bjpop/methpat](https://github.com/bjpop/methpat)).
