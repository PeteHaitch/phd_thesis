---
title: "A statistical framework for analysing whole-genome bisulfite-sequencing data"
output:
  pdf_document:
    fig_caption: yes
    includes:
...
\chapter{A statistical framework for analysing whole-genome bisulfite-sequencing data}\label{chap:wgbs_statistical_framework}
...
We denote the methylation pattern on the $h^{th}$ DNA fragment containing the m-tuple $(i, i + 1, \ldots, i + \text{m} - 1)$ by the vector of indicator random variables, $Z_{h, (i, i + 1, \ldots, i + \text{m} - 1)}$,:

\begin{equation*}
Z_{h, (i, i + 1, \ldots, i + \text{m} - 1)} = \left\{
  \begin{array}{l l}
  (0, 0, \ldots, 0) & \quad \text{if unmethylated at the } \Big (i^{th}, \ldots, (i + \text{m} - 1)^{th} \Big ) \\
  & \quad \text{locus on the } h^{th} \text{ fragment}\\
  (0, 0, \ldots, 1) & \quad \text{if unmethylated at the } \Big (i^{th}, \ldots, (i + \text{m} - 2)^{th} \Big ) \\
  & \quad \text{locus and methylated at the } (i + \text{m} - 1)^{th} \\
  & \quad \text{locus on the } h^{th} \text{ fragment} \\
  \vdots \\
  (1, 1, \ldots, 1) & \quad \text{if methylated at the } \Big (i^{th}, \ldots, (i + \text{m} - 1)^{th} \Big ) \\
  & \quad \text{locus on the } h^{th} \text{ fragment}
  \end{array} \right.
  \end{equation*}
...
A whole-genome bisulfite-sequencing experiment does not sequence every DNA fragment in the pool. Rather, sequencing can be thought of as sampling without replacement from the pool of DNA fragments. We have a large number (~$10^{10}$) of fragments in the pool and each methylation locus is only present on a small number of those fragments. We can therefore approximate this sampling by Poisson sampling, where the rate parameter for locus $i$ is proportional to the number of fragments in the pool and inversely proportional to the number of fragmentation containing the $i^{th}$ methylation locus, $H_{i}$.
...
2. Loci may be measured twice if the insert size is less than the sum of the read lengths, e.g. _read-1_ gives us $(i, i + 1)$ and _read-2_ gives us $(i + 1, i + 2, i + 3)$. In this example we must use only one of _read-1_ or _read-2_ as the measurement of the $(i + 1)^{th}$ locus because otherwise we are 'double-counting'[^overlapping-mates].

[^overlapping-mates]: We should also check that the overlapping bases agree and, if not, either use the call with the highest base quality or ignore these overlapping positions in both reads. `methtuple` has several options for handling overlapping mates of paired-end reads via the `--overlap-filter` flag. The default method is to ignore any read-positions in the overlapping region where the methylation calls disagree.

Each read measures the methylation state of one or more loci from a single DNA fragment. We denote by $\mathcal{R}_i$ the set of all mapped reads containing the $i^{th}$ locus. The number of reads containing the $i^{th}$ locus is referred to as the sequencing depth at the $i^{th}$ locus, which we denote by $d_{i} = |\mathcal{R}_{i}|$, where $d_{i} \leq H_{i}$ with strict inequality for almost all $i$.
...
This is the so-called $\beta$-value, which is commonly interpreted as an estimate of $B_{i}$, the proportion of cells in the sample that are methylated at the $i^{th}$ locus. In Section \ref{sec:estimating_B} we discuss this interpretation and other estimators of the 'methylation level' at a locus.
...
As we did for $B$-values, we can extend the $\beta$ values to m-tuples. Here are the definitions for $\text{m} = 2$:
...
\subsection{Some complications}\label{sec:one_sample_complications}
...
\subsection{Some complications}\label{sec{multi_sample_complications}}
...
Both `bsseq` and `BiSeq` use a binomial local likelihood smoother. In each case this smoother is chosen because `BSmooth` and `BiSeq` model the number of methylated reads at the $i^{th}$ locus in the $j^{th}$ sample by $M_{i, j} | d_{i, j} \eqd Binomial(d_{i, j}, B_{i, j})$. The smoothing is 'local' to leverage co-methylation, which is considered a local phenomenon.
...
[^rrbs_sim]: Both \citet{Hebestreit:2013ko} and \citet{Lacey:2013iy} altered the default `bsseq` parameters to try to make them comparable to `BiSeq`. \citet{Hebestreit:2013ko} changed the default minimum window size to $80$ bp but still required at least $20$ CpGs per window. \citet{Lacey:2013iy} kept the default minimum window size of $2,000$ bp but reduced the minimum number of CpGs per window to $50$ from the default of $70$. Nevertheless, the fact remains that `bsseq` is designed for analysing whole-genome bisulfite-sequencing data and not RRBS, which puts it at a disadvantage in these comparisons.

\section{Statistical properties of $\beta$-values}\label{sec:beta}

Under the (conditional) binomial model, $M_{i, j} | d_{i, j} \eqd Binomial(d_{i, j}, B_{i, j})$, $\beta_{i, j} = \frac{m_{i, j}}{d_{i, j}}$ is an unbiased estimator of $B_{i, j}$ with standard error $se(\beta_{i, j}) = \sqrt{\frac{\beta_{i, j}(1 - \beta_{i, j})}{d_{i, j}}}$ \citep{Hansen:2012gr}. The natural interpretation of $\beta_{i, j}$ is then as an estimator of the average level of methylation at the $i^{th}$ locus in the $j^{th}$ sample. In this section I discuss this interpretation and statistical properties of this estimator.

\subsection{Empirical distributions of $\beta$-values}\label{sec:empirical-distributions-of-beta-values}
...
What is immediately clear is that these distributions are bimodal: most CpGs are highly methylated or lowly methylated. The exception is the _E16.5\_male\_1_ sample from the _Seisenberger_ data which is hypomethylated and with an enormous number of intermediately methylated CpGs. The _E16.5\_male\_1_ sample is a progenitor germ cells from a pool of embryonic day 16.5 male mice. Between days E6.5 and E13.5, the mouse progenitor germ cells undergo global demethylation and it is only from day E16.5 onwards that they begin to be _de novo_ methylated \citep{Seisenberger:2012ko}, hence the wide variation in $\beta$-values.
...
CpG $\beta$-values are often computed by aggregating the $m$ and $u$ counts across the forward and reverse strands. On average, this doubles the sequencing coverage of each CpG but presupposes that the two strands are indeed symmetrically methylated. To investigate the validity of this assumption we can compute the correlation of strand-specific $\beta$-values. Figures \ref{fig:EPISCOPE_strand_correlations}, \ref{fig:Lister_strand_correlations}, \ref{fig:Seisenberger_strand_correlations} and \ref{fig:Ziller_merged_strand_correlations} report the Pearson correlation of these strand-specific $\beta$-values for varying sequencing coverage cutoffs.

\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{../figures/EPISCOPE_strand_correlations.pdf}
\caption[\emph{EPISCOPE}: Correlations of $\beta$-values across strands]{Correlations of $\beta$-values across strands for the \emph{EPISCOPE} dataset using different minimum sequencing coverage cutoffs.}
\label{fig:EPISCOPE_strand_correlations}
\end{figure}

\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{../figures/Lister_strand_correlations.pdf}
\caption[\emph{Lister}: Correlations of $\beta$-values across strands]{Correlations of $\beta$-values across strands for the \emph{Lister} dataset using different minimum sequencing coverage cutoffs.}
\label{fig:Lister_strand_correlations}
\end{figure}

\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{../figures/Seisenberger_strand_correlations.pdf}
\caption[\emph{Seisenberger}: Correlations of $\beta$-values across strands]{Correlations of $\beta$-values across strands for the \emph{Seisenberger} dataset using different minimum sequencing coverage cutoffs.}
\label{fig:Seisenberger_strand_correlations}
\end{figure}

\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{../figures/Ziller_merged_strand_correlations.pdf}
\caption[\emph{Ziller: Correlations of $\beta$-values across strands}]{Correlations of $\beta$-values across strands for the \emph{Ziller} dataset using different minimum sequencing coverage cutoffs.}
\label{fig:Ziller_merged_strand_correlations}
\end{figure}
...
Some of the embryonic stem cell samples (_H1\_r1_, _H1\_r2_ and _HSF1_ from the _Lister_ dataset) have less correlated strand-specific $\beta$-values, $r = 0.5$ to $0.7$. The other embryonic stem cell samples have higher correlations of $\beta$-values across strands, although the estimates are quite different between the two replicates of the same cell line (_H9_: $r = 0.77$, _H9\_Laurent_: $r = 0.92$). This suggests that caution may be warranted in combining CpG methylation levels across strands for embryonic stem cell samples.

All three _Seisenberger_ samples have noticeably less correlated strand-specific $\beta$-values, including the embryonic stem cell, _J1\_1_. However, since these data are from pooled DNA, the source of this reduced correlation is difficult to identify.
