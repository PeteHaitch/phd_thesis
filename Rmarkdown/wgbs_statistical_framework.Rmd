...
z: z \in \mathcal{R}_{(i, i + 1, \ldots, i + \text{m} - 1)} = \left\{
  \begin{array}{l l}
    ...
    & \quad \Big ( i^{th}, (i + 1)^{th}, \ldots, (i + \text{m} - 1)^{th} \Big) \text{ locus}\\
    (0, 0, \ldots, 1) & \quad \text{if unmethylated at the } \\
    \quad \Big (i^{th}, (i + 1)^{th}, \ldots, (i + \text{m} - 2)^{th} \Big) \text{ locus} \\
...
__TODO: Update discussion of results in light of Ziller merged data__
...
stringsAsFactors = FALSE)
colnames(x) <- 1:10
xt <- xtable(x, caption = "Correlations of $\\beta$-values across strands for the Ziller dataset. Each row is a sample and each column is a minimum coverage cutoff.", label = "tab:Ziller_merged_strand_correlations")
align(xt) <- rep("c", ncol(x) + 1)
digits(xt) <- 2
print(xt, booktabs = TRUE, comment = FALSE, tabular.environment = "longtable", floating = FALSE)
```
