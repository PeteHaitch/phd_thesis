...
In practice, the stitching together of segments is not strictly a separate step and can be incorporated into step 2.

\subsubsection{Segmenting the methylome}

- [x] DONE

`methsim` uses the the R/Bioconductor package, `MethylSeekR` \citep{Burger:2013kq}, to segment the methylome into these regions of similarity. Initially published to discover regulatory motifs from bisulfite-sequencing data, `MethylSeekR` segments the genome into unmethylated regions (_UMR_s), lowly-methylated regions (_LMR_s) and partially methylated regions[^pmds] (_PMR_s). It does this using a two-stage algorithm applied to the $\beta$-values from a sample:
...
1. Identify partially methylated regions. A summary statistic, $\alpha$, (__TODO: What is $\alpha$__) which is based on the $\beta$-values in a sliding window of 100 CpGs, is used to identify PMRs. Briefly, a two-state hidden Markov model is fit to the $\alpha$ values to identify _PMR_s and non-_PMR_s
...
`methsim` uses a two-state ($1 =$ methylated or $0 =$ unmethylated), one-step Markov chain to model $Z_{h, j}$[^methsim_model]. This allows the incorporation of within-fragment co-methylation into the simulation.  The choice of a one-step chain is one of computational simplicity, although I will show that this is a reasonable approximation of within-fragment co-methylation.

[^methsim_model]: In fact, as we will see, `methsim` does not directly model $Z_{h, j}$, but rather models a kind of hybrid process.
...
[^context]: Actually, it only depends on the context of the $i^{th}$ locus. Most pairs of loci, $(i, i + 1)$, will lie in the same context. For pairs that span the boundary of two different contexts I have arbitrarily chosen to use the context of the $i^{th}$ locus.
...
`methsim` assigns these transition probabilities based on empirical the distributions of $\beta_{i, j}$ and $LOR_{IPD(i, i + 1), j}$ conditional on the region type of the $i^{th}$ locus, $r_{i}$. Call these empirical distributions $\beta_{i, j | r_{i}}$ and $LOR_{i, j | r_{i}}$, respectively. These empirical distributions are shown for the ADS in Figures \ref{fig:ADS_beta_by_pm_boxplots} and \ref{fig:ADS_lor_by_pm_quantile_plots}

\begin{figure}[!htb]
\centering
\centering
\includegraphics[width=\textwidth]{../figures/ADS_beta_by_pm_boxplots.pdf}
\caption{$\beta_{i, j | r_{i}}$ and $LOR_{i, j | r_{i}}$, the distribution of $\beta$-values for CpGs in each region type, for the _ADS_ sample. Only CpGs with at least $10\times$ sequencing coverage are used. UMR = unmethylated region; LMR = lowly methylated region; PMR = partially methylated region; other = any other region.}
...
\caption{$\beta_{i, j | r_{i}}$ and $LOR_{i, j | r_{i}}$, the distribution of $\beta$-values for CpGs in each region type, for the EPISCOPE dataset. Only CpGs with at least $10\times$ sequencing coverage are used. UMR = unmethylated region; LMR = lowly methylated region; PMR = partially methylated region; other = any other region.}
...
\caption{$LOR_{i, j | r_{i}}$ and $LOR_{i, j | r_{i}}$, the distribution of $\log_{2}$ odds-ratio estimates of within-fragment co-methylation for CpGs in each region type, for the _EPISCOPE_ sample. Only 2-tuples with at least $10\times$ sequencing coverage are used. UMR = unmethylated region; LMR = lowly methylated region; PMR = partially methylated region; other = any other region.}
...
\caption{$\beta_{i, j | r_{i}}$ and $LOR_{i, j | r_{i}}$, the distribution of $\beta$-values for CpGs in each region type, for the Lister dataset. Only CpGs with at least $10\times$ sequencing coverage are used. UMR = unmethylated region; LMR = lowly methylated region; PMR = partially methylated region; other = any other region.}
...
\caption{$LOR_{i, j | r_{i}}$ and $LOR_{i, j | r_{i}}$, the distribution of $\log_{2}$ odds-ratio estimates of within-fragment co-methylation for CpGs in each region type, for the _Lister_ sample. Only 2-tuples with at least $10\times$ sequencing coverage are used. UMR = unmethylated region; LMR = lowly methylated region; PMR = partially methylated region; other = any other region.}
...
\caption{$\beta_{i, j | r_{i}}$ and $LOR_{i, j | r_{i}}$, the distribution of $\beta$-values for CpGs in each region type, for the Seisenberger dataset. Only CpGs with at least $10\times$ sequencing coverage are used. UMR = unmethylated region; LMR = lowly methylated region; PMR = partially methylated region; other = any other region.}
...
\caption{$LOR_{i, j | r_{i}}$ and $LOR_{i, j | r_{i}}$, the distribution of $\log_{2}$ odds-ratio estimates of within-fragment co-methylation for CpGs in each region type, for the _Seisenberger_ sample. Only 2-tuples with at least $10\times$ sequencing coverage are used. UMR = unmethylated region; LMR = lowly methylated region; PMR = partially methylated region; other = any other region.}
...
\caption{$\beta_{i, j | r_{i}}$ and $LOR_{i, j | r_{i}}$, the distribution of $\beta$-values for CpGs in each region type, for the Ziller dataset. Only CpGs with at least $10\times$ sequencing coverage are used. UMR = unmethylated region; LMR = lowly methylated region; PMR = partially methylated region; other = any other region.}
...
\caption{$LOR_{i, j | r_{i}}$ and $LOR_{i, j | r_{i}}$, the distribution of $\log_{2}$ odds-ratio estimates of within-fragment co-methylation for CpGs in each region type, for the _Ziller_ sample. Only 2-tuples with at least $10\times$ sequencing coverage are used. UMR = unmethylated region; LMR = lowly methylated region; PMR = partially methylated region; other = any other region.}
\label{fig:Ziller_merged_lor_by_pm_quantile_plots}
\end{figure}

__UP TO HERE: Describe how a transition probability can be computed from a marginal probability and a log odds ratio__
