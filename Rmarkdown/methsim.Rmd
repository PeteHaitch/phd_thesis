...
\subsubsection{Simulating $\mathbf{{Z_{\tilde{h}}}}$}
...
`methsim` assigns these transition probabilities based on $\beta_{i | r_{i}}$ and $LOR_{(i, i+1) | r_{i}, IPD(i, i+1)}$, the empirical distributions of $\beta_{i}$ and $LOR_{IPD(i, i + 1)}$ conditional on the region type of the $i^{th}$ locus, $r_{i}$, and the intra-pair distance of the the $i^{th}$ and $(i + 1)^{th}$ locus, $IPD(i, i+1)$, respectively. These empirical distributions are shown for the \emph{ADS} sample in Figures \ref{fig:ADS_beta_by_pm_boxplots} and \ref{fig:ADS_lor_by_pm_quantile_plots} (__TODO: Are these other examples necessary?__)

\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{../figures/ADS_beta_by_pm_boxplots.pdf}
\caption{$\beta_{i | r_{i}}$ and $LOR_{i | r_{i}}$, the distribution of $\beta$-values for CpGs in each region type, for the \emph{ADS} sample. Only CpGs with at least $10\times$ sequencing coverage are used. UMR = unmethylated region; LMR = lowly methylated region; PMR = partially methylated region; other = any other region.}
...
\caption{$LOR_{i | r_{i}}$ and $LOR_{i | r_{i}}$, the distribution of $\log_{2}$ odds-ratio estimates of within-fragment co-methylation for CpGs in each region type, for the \emph{ADS} sample. Only 2-tuples with at least $10\times$ sequencing coverage are used. UMR = unmethylated region; LMR = lowly methylated region; PMR = partially methylated region; other = any other region.}
\label{fig:ADS_lor_by_pm_quantile_plots}
\end{figure}

These same distributions for all the _Ziller_, _Lister_, _Seisenberger_ and _Ziller_ datasets are shown in Figures \ref{fig:EPISCOPE_beta_by_pm_boxplots}, \ref{fig:EPISCOPE_lora_by_pm_quantile_plots}, \ref{fig:Lister_beta_by_pm_boxplots}, \ref{fig:Lister_lora_by_pm_quantile_plots},
...
\caption{$\beta_{i | r_{i}}$ and $LOR_{i | r_{i}}$, the distribution of $\beta$-values for CpGs in each region type, for the \emph{EPISCOPE} dataset. Only CpGs with at least $10\times$ sequencing coverage are used. UMR = unmethylated region; LMR = lowly methylated region; PMR = partially methylated region; other = any other region.}
...
\caption{$\beta_{i | r_{i}}$ and $LOR_{i | r_{i}}$, the distribution of $\beta$-values for CpGs in each region type, for the \emph{Lister} dataset. Only CpGs with at least $10\times$ sequencing coverage are used. UMR = unmethylated region; LMR = lowly methylated region; PMR = partially methylated region; other = any other region.}
...
\caption{$\beta_{i | r_{i}}$ and $LOR_{i | r_{i}}$, the distribution of $\beta$-values for CpGs in each region type, for the \emph{Seisenberger} dataset. Only CpGs with at least $10\times$ sequencing coverage are used. UMR = unmethylated region; LMR = lowly methylated region; PMR = partially methylated region; other = any other region.}
...
\caption{$\beta_{i | r_{i}}$ and $LOR_{i | r_{i}}$, the distribution of $\beta$-values for CpGs in each region type, for the \emph{Ziller} dataset. Only CpGs with at least $10\times$ sequencing coverage are used. UMR = unmethylated region; LMR = lowly methylated region; PMR = partially methylated region; other = any other region.}
...
1. For each region, $r_{i}$, sample $B_{r_{i}}$ from $\beta_{i | r_{i}}$. This can be interpreted as $Pr(Z_{r_{i}} = 1)$, the marginal probability that a methylation locus in region $r_{i}$ is methylated, or, alternatively, as the average level of methylation for all methylation loci in the region.
...
Now that we have $B_{r_{i}}$ and $\psi_{i, i + 1}$ for $1 \leq i \leq n - 1$ we can simulate realisations of $\mathbf{Z_{\tild{h}}}$. To begin the process (i.e., when $i = 1$) $Z_{\tilde{h}, 1}$, is simulated from the "marginal" distribution, which is Bernoulli($B_{r_{1}}$). Then, we loop over $i = 2, \ldots, n - 1$ to simulate the remainder of the process. At each step we compute the transition probability, $p_{i, i + 1} = Pr(Z_{\tilde{h}, i + 1} = 1 | Z_{\tilde{h}, i}) = \frac{Pr(Z_{tilde{h}, i}, Z_{tilde{h}, i + 1} = 1)}{Pr(Z_{tilde{h}})}$, from $B_{r_{i}}, $B_{r_{i + 1}}, \psi_{(i, i + 1)}$ and the observed (simulated) value at the previous iteration, $Z_{\tilde{h}, i}$. Then, $Z_{\tilde{h}, i + 1}$ is simulated from the Bernoulli($p_{i, i + 1}$) distribution.

To compute the transition probability, $p_{i, i + 1}$, `methsim` uses the iterative proportional fitting algorithm (IPF; __TODO: CITE IPD paper__) to construct the joint probability matrix, __TODO: Mathematically describe this matrix__. Then, the appropriate element of this matrix is divided by the appropriate marginal probability to obtain the transition probability. An example of this procedure is illustrative.

The IPD procedure is a general method for __TODO: blah blah__. In this case we wish to use the IPF procedure to construct the unique $2 \times 2$ array (the joint probability matrix) with specified margins (the marginal probabilities of each methylation state at each locus) and the specified cross-product ratio (the odds-ratio). __UP TO HERE__

\begin{equation}
% An example of IPF being used to form a transition matrix.
\end{equation}
