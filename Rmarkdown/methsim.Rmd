...
For simplicity, `methsim` assumes that all $n_{\tilde{h}}$ realisations have the same parameters (described below; __TODO: Discuss implications of this assumption somewhere below__).
...
1. Identify partially methylated regions. A summary statistic, $\alpha$, (__TODO: What is $\alpha$__) which is based on the $\beta$-values in a sliding window of 100 CpGs, is used to identify PMRs. Briefly, a two-state hidden Markov model is fit to the $\alpha$ values to identify PMRs and non-PMRs.
...
\ref{fig:Ziller_merged_beta_by_pm_boxplots} and \ref{fig:Ziller_merged_lor_by_pm_quantile_plots} (__TODO: Are these other examples necessary?__).
...
The second step requires further explanation. If there are an insufficient number of observations with the given $IPD$-region combination, then, by default, `methsim` samples $\Psi_{(i, i + 1)}$ from a Gaussian distribution with mean equal to __TODO: What?__ and standard deviation equal to __TODO: What?__. __TODO: Explain how these means and standard deviations are estimated using loess.__ By default, if there are less than __TOOD: how many?__ $LOR$ with the given $IPD(i, i + 1), r_{i}$ combination then the Normal-approximation is used. The user may specify a different method for dealing with these rarer $IPD$-region combinations.
...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{../figures/Ziller_pattern_freqs_by_pm_boxplots.pdf}
...
\label{fig:Ziller_pattern_freqs_by_pm_boxplots}
\end{figure}

From Figures \ref{fig:EPISCOPE_pattern_freqs_by_pm_boxplots}, \ref{fig:Lister_pattern_freqs_by_pm_boxplots}, \ref{fig:Seisenberger_pattern_freqs_by_pm_boxplots} and \ref{fig:Ziller_pattern_freqs_by_pm_boxplots} we see that, unsurprisingly, in UMRs that a single methylation pattern dominates, the completely unmethylated pattern. Outside of UMRs, we see that while there is generally a dominant methylation pattern at each m-tuple, there may be $2-4$ otheres that are observed at non-negligible frequencies. PMRs are the most heterogeneous and LMRs and other regions are quite similar.
...
With all the above limitations in mind, `methsim` still uses the simple strategy of sampling the $\mathbf{W}$ from $w_{(i, \ldots, i + m - 1) | r_{(i, \ldots, i + m - 1)}}$. At present I do not have a better alternative and, as we shall see, the current procedure still procedure still produced realistic simulated data.
