...
The second step requires further explanation. If there are an insufficient number of observations with the given $IPD$-region combination, then, by default, `methsim` samples $\Psi_{(i, i + 1)}$ from a Gaussian distribution[^loess_params]. By default, if there are less than __TOOD: how many?__ $LOR$ with the given combination of region-type and IPD then the Gaussian-approximation is used. The user may specify a different method for dealing with these rarer $IPD$-region combinations and, indeed, different methods are explored in the __RESULTS SECTION__.

[^loess_params]: The mean, $\mu$, and standard deviation, $\sigma$, of this Gaussian distribution depend on both $IPD$ and $r_{i}$. A loess fit of $mean(LOR)$ (resp. $sd(LOR)$) as a function of $IPD$ is performed, stratified by region type. The values of $\mu$ (resp. $\sigma$) are then obtained by extrapolating the loess fit (__TODO: Update if changes made to `methsim` defaults__).
