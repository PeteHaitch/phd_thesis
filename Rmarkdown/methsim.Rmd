...
To simulate these type of experiments, we need to be able to control the variation between the `SimulatedWGBS` objects. In this example, we probably want the three samples in each group to be more similar to each other than to the three samples in the other group. Essentially, we need to be able to control the within-group and between-group variation.
...
We might also consider more subtle ways of introducing variation. For example, we might use the same `MethylomeParam` object but vary how we simulate $\mathbf{B}$ and $\mathbf{\psi}$ between calls to `simulate()`.


and the same average methylation level for each region but vary the strength of co-methylation. This could be achieved by altering the $LOR_{(i, i+1) | r_{i}, IPD(i, i+1)}$ distributions between two `SimulateMethylomeParam` objects while holding the rest constant. Or we might vary the weights, $W$, to simulate more or less heterogeneous samples.

In Section ref{sec:results} I explore how much variation is created using these different techniques.
...
Shown below are the running times and memory usage involved in simulating a $30\times$ whole-genome bisulfite-sequencing sample using the _ADS_ `MethylomeParam` object included in `methsim`. Using up to 8 CPU cores in parallel, it takes less than 5 minutes to simulate the underlying 'true' methylome and a further __HOW LONG__ to simulate a single $30 \times$ sequencing coverage whole-genome bisulfite-sequencing assay of this methylome[^parallel].

[^parallel]: NB the creation of the `SimulatedMethylome` via `simulate(ADS, BPPARAM)` currently only uses a single CPU.

__UP TO HERE: simulate-WGBSParam-method seems to be causing a memory leek and resulting crash. Running valgrind in reduced problem to investigate.__
...
# ADS: the MethylomeParam object.
pryr::object_size(ADS)
#> 16.5 MB

# sm: the SimulatedMethylome object
system.time(l_sm <- simulate(ADS, BPPARAM = BPPARAM))
#> Currently only simulates CpG methylation.
#> Currently only simulates unstranded methylomes.
#> Simulating 1 methylome..
#>    user  system elapsed
#> 186.680   9.676 270.780
sm <- l_sm[[1]]
pryr::object_size(sm)
#> 676 MB

# wgbsp: the WGBSParam object.
# NOTE: Basically all the memory used by the WGBSParam object is the
# SimulatedMethylome object.
wgbsp <- WGBSParam(SimulatedMethylome = sm,
                   AveCov = 30,
                   ErrorRate = 0.01,
                   SequencingType = 'SE',
                   ReadLength = 100)
pryr::object_size(wgbsp)
#> 676 MB

# sbs: the SimulatedBS object.
system.time(sbs <- simulate(wgbsp, BPPARAM = BPPARAM))
#> Simulating 1 bisulfite-sequencing sample...
