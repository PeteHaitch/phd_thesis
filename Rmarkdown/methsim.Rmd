...
`methsim` is written in R and builds on the `MethylationTuples` package described in __CHAPTER__, as well as key R packages available on Bioconductor and CRAN (see `DESCRIPTION` for a complete list. __TODO: Add citations of all packages used in thesis__).
...
The next step is to simulate a 'true' methylome by applying the `simulate()` method to the `SimulateMethylomeParam` object. This returns a `SimulatedMethylome`, a schematic of which is __SHOWN BELOW__.
