...
\caption[\emph{EPISCOPE}: $\beta$-values my region-type]{Boxplots of CpG $\beta$-values in each region-type following segmentation by \texttt{MethylSeekR} for the \emph{EPISCOPE} dataset.}
...
\caption[\emph{Lister}: $\beta$-values my region-type]{Boxplots of CpG $\beta$-values in each region-type following segmentation by \texttt{MethylSeekR} for the \emph{Lister} dataset.}
...
\caption[\emph{Seisenberger}: $\beta$-values my region-type]{Boxplots of CpG $\beta$-values in each region-type following segmentation by \texttt{MethylSeekR} for the \emph{Seisenberger} dataset.}
...
\caption[\emph{Ziller}: $\beta$-values my region-type]{Boxplots of CpG $\beta$-values in each region-type following segmentation by \texttt{MethylSeekR} for the \emph{Ziller} dataset.}
...
The result of this process is a `SimulatedWGBS` object storing the ID of each read that overlaps a methylation locus, along with the corresponding genomic co-ordinates and the methylation states.
...
The `SimulatedMethylome` and `SimulatedWGBS` objects are large in memory. This is to some extent unavoidable; we are simulating whole-genome bisulfite-sequencing sequencing experiments that produce an enormous amount of data.
