...
This workflow is illustrated in __FIGURE__ and described in the remainder of this section.
...
`methsim` makes extensive use of the S4 object system in R. The most important classes defined by `methsim` are `SimulateMethylomeParam`, `SimulatedMethylome`, `SimulateBSParam` and `SimulatedBS`. The most important methods are the `simulate()` methods defined for the `SimulateMethylomeParam` and `SimulateBSParam` classes. The role of these classes and methods in the workflow is illustrated in __FIGURE__.

The `SimulateMethylomeParam` object contains the empirical distributions of key statistics from which the parameters for simulating the 'true' methylome are sampled. Therefore, the `SimulateMethylomeParam` should be based on a relevant sample. To help a new user get started, `methsim` includes `SimulateMethylomeParam` objects for the _ADS_, _ADS-adipose_ and _ADS-iPSC_ samples from the _Lister_ dataset. Alternatively, the user may process the BAM file for their sample with `methtuple` and then use the helper functions in `methsim` to construct a `SimulateMethylomeParam` object based on their sample of interest.
...
To simulate bisulfite-sequencing reads, the user first constructs a `SimulateBSParam`, which includes the `SimulatedMethylome`. The user then runs the `simulate()` method on the `SimulatedBSParam` object to create a simulated bisulfite-sequencing data in the form of a `SimulatedBS` object.

Finally, we may wish to simplify the `SimulatedBS` object. For example, we may only require the $\beta$-values for our downstream analysis and not the  read-level data. `methsim` includes the `asMethPat()` function to convert the `SimulatedBS` object to a `MethPat` object (described in __CHAPTER__).

__UP TO HERE: See if the remainder of this section needs re-jigging in light of the above changes to Implementation__
