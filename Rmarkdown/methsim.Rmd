...
Under the Beta-Binomial model, $\mu_{i, j}$ is the mean and $\phi_{i, j}$ the dispersion of the Beta distribution[^beta_parameterisation]. The means, $\mu_{i, j}$, are group-specific while the dispersions, $\phi_{i, j}$, are sample specific. If $\mu_{i, j}$ are allowed to be sample-specific then there would be no group-structure in the subsequent $\beta$-values. Conversely, if the dispersions are group-specific then there would be little within-group variability in the subsequent $\beta$-values.
