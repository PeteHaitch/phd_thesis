...
These same distributions for all the _Ziller_, _Lister_, _Seisenberger_ and _Ziller_ datasets are shown in Figures \ref{fig:EPISCOPE_beta_by_pm_boxplots}, \ref{fig:EPISCOPE_lor_by_pm_quantile_plots}, \ref{fig:Lister_beta_by_pm_boxplots}, \ref{fig:Lister_lor_by_pm_quantile_plots},
...
For each region we need to assign the vector of weights, $\mathbf{W_{r_{i}, \cdot}}$. To do so, `methsim` uses a heuristic procedure. For higher-order m-tuples, $m = 3$ or $4$ is recommended, `methsim` computes the relative frequency of the $2^{m}$ possible methylation patterns at each m-tuple with sufficient sequencing coverage. `methsim` then samples from this distribution, stratified by region-type; call these empirical distributions

__TODO__: Really computing (and plotting) $w_{i | r_{(i, \ldots, i + m - 1)}}, \ldots, w_{i + m - 1 | r_{(i, \ldots, i + m - 1)}}$ not $w_{(i, \ldots, i + m - 1) | r_{(i, \ldots, i + m - 1)}}$. These empirical distributions are shown for the \emph{ADS} sample in Figure \ref{fig:ADS_pattern_freqs_by_pm_boxplots}. From this we see that __TODO__.

__TODO: Update figures to use $w_{1}, \ldots, w_{8}$ rather than $h1, \ldots h8$.__

\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{../figures/ADS_pattern_freqs_by_pm_boxplots.pdf}
\caption{$w_{(i, \ldots, i + m - 1) | r_{(i, \ldots, i + m - 1)}}$, the distribution of the relative frequencies for all CpG 3-tuples in each region type, for the \emph{ADS} sample. Only CpGs with at least $5\times$ sequencing coverage are used. UMR = unmethylated region; LMR = lowly methylated region; PMR = partially methylated region; other = any other region.}
\label{fig:ADS_beta_by_pm_boxplots}
\end{figure}

__TODO: Limitations of this procedure__
