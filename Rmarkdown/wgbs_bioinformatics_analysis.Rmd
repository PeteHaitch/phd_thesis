...
xt <- xtable(x, caption = c("Percentage of dinucleotides that are CpGs for each chromosome in the human reference genome (hg19).", "CpG percentage per chromosome in hg19"), label = "tab:cpg_density_hg19")
align(xt) <- rep("c", ncol(x) + 1)
digits(xt) <- 2
print(xt, booktabs = TRUE, comment = FALSE, tabular.environment = "longtable", floating = FALSE, include.rownames = FALSE, caption.placement = "top")
```
